 ...
 */
package tahwissa.desktop.modifierrando;

import tahwissa.desktop.organiser.*;
...
import tahwissa.desktop.DrawerController;
 ...
 * @author Meedoch
 */
public class ModifierController implements Initializable, MapComponentInitializedListener {
    private List<Creneau> creneauxToDelete = new ArrayList<>();
    ...
    public void mapInitialized() {
        ...
        geoService.geocode(event.getDestination(), (grs, gs) -> {
            if (gs.equals(gs.OK)) {
                ...
                infoOpt.content(event.getDestination());
    ...
    void initValues() {
        ...
        nbPlaces.setText(String.valueOf(event.getNombrePlaces()));
        frais.setText(String.valueOf(event.getFrais()));
        lieuRassemblement.setText(event.getLieuRassemblement());
        LocalDateTime dt = event.getDateHeureDepart().toLocalDateTime();
        LocalDate d = dt.toLocalDate();
        dateDepart.setValue(d);
        //heureDepart.setTime(dt.toLocalTime());
        if (event.getEvenementType().equals("randonnee")) {
            randonnee.setSelected(true);
            randonneeSelected(null);
            typeRando.setText(event.getType());
            distanceParcourue.setText(event.getDistanceParcourue().toString());
            System.out.println(event.getDifficulte());
            if (event.getDifficulte().equals("Facile")) {
                difficulte.getSelectionModel().select(0);
            } else if (event.getDifficulte().equals("Moyenne")) {
                difficulte.getSelectionModel().select(1);
            } else {
                difficulte.getSelectionModel().select(2);
            }
        } else {
            camping.setSelected(true);
            campingSelected(null);
            nbj.setText(String.valueOf(event.getDuree()));
        }
        
        planning = event.getPlanning();
        
        dateDebut0.setValue(planning.get(0).getDateDebut().toLocalDateTime().toLocalDate());
//        heureDebut0.setTime(planning.get(0).getDateDebut().toLocalDateTime().toLocalTime());
        dateFin0.setValue(planning.get(0).getDateFin().toLocalDateTime().toLocalDate());
        //heureFin0.setTime(planning.get(0).getDateFin().toLocalDateTime().toLocalTime());
        description0.setText(planning.get(0).getDescription());
        for (int i=1;i<planning.size();i++){
            addCreneau(null);
            datesDebut.get(i).setValue(planning.get(i).getDateDebut().toLocalDateTime().toLocalDate());
           // heuresDebut.get(i).setTime(planning.get(i).getDateDebut().toLocalDateTime().toLocalTime());
            datesFin.get(i).setValue(planning.get(i).getDateFin().toLocalDateTime().toLocalDate());
           // heuresFin.get(i).setTime(planning.get(i).getDateFin().toLocalDateTime().toLocalTime());
            descriptions.get(i).setText(planning.get(i).getDescription());
        }
...
//
//        service.modifierEvenement(event, files, planning);
//        Notification.createNotification("Succès", "Votre demande d'organisation a été modifiée");
